...
def calc_std(std_dir, hsv, width, height):
    std_file_count = len([name for name in os.listdir(std_dir)])
    std_values_r = np.ndarray((std_file_count + 1, height, width), int)
    std_values_g = np.ndarray((std_file_count + 1, height, width), int)
    std_values_b = np.ndarray((std_file_count + 1, height, width), int)
    count = 0
    for file in os.listdir(std_dir):
        std = cv2.imread(std_dir + '\\' + file, cv2.IMREAD_COLOR)
        std=perspective_change.get_perspective_change(None,img=std)
        plt.imshow(std)
        if hsv:
            std = cv2.cvtColor(std, cv2.COLOR_BGR2HSV)
        [std_1, std_2, std_3] = cv2.split(std)
        std_1 = cv2.resize(std_1, (width, height))
        std_2 = cv2.resize(std_2, (width, height))
        std_3 = cv2.resize(std_3, (width, height))
...
def foreground_poc(threshold, diff_img, new_width, new_height,std_dir=None):
    ...
    diff_r = cv2.resize(diff_r, (new_width, new_height))
    diff_g = cv2.resize(diff_g, (new_width, new_height))
    diff_b = cv2.resize(diff_b, (new_width, new_height))
    if std_dir is not None:
        calc_std(std_dir, hsv,width=new_width,height=new_height)
